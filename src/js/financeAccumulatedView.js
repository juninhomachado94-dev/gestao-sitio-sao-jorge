...
setInterval(() => {
  if (document.querySelector(".finance-page")) {
    enhanceFinancePage();
  } else {
    state.lastSignature = "";
  }
...
function scheduleEnhance(delay = 0) {
  window.setTimeout(enhanceFinancePage, delay);
}
...
function enhanceFinancePage() {
  ...
  const signature = buildSignature(page);
...
function buildSignature(page) {
  const finance = normalizeFinance(getFinanceData());
  const fixed = safeArray(getFixedExpenses());
